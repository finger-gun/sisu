...
The `@sisu-ai/tool-vec-chroma` package SHALL expose a high-level retrieval tool that accepts plain query text and performs semantic retrieval without requiring callers to precompute embeddings.
...
- **THEN** the tool MUST embed the query text, execute vector retrieval, and return compact retrieval results
