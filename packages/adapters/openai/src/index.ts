...
type OpenAIResponse = {
  choices?: Array<{ message?: OpenAIMessageShape }>;
...
export function openAIEmbeddings(
  ...
  const apiKey =
    opts.apiKey ?? firstConfigValue(["API_KEY", "OPENAI_API_KEY"]) ?? "";
  const envBase = firstConfigValue(["BASE_URL", "OPENAI_BASE_URL"]);
  const baseUrl = resolveBaseUrl(opts.baseUrl, envBase, "https://api.openai.com");
  const defaultModel = opts.model || "text-embedding-3-small";
  if (!apiKey) {
    throw new Error(
      "[openAIEmbeddings] Missing API_KEY or OPENAI_API_KEY — set it in your environment or pass { apiKey }",
    );
  }
...
export function openAIAdapter(opts: OpenAIAdapterOptions): LLM {
  ...
  const generate = ((
    ...
    const body: Record<string, unknown> = {
      ...
      ...(toolsParam.length && tool_choice !== undefined
        ? { tool_choice }
        : {}),
      ...(genOpts?.parallelToolCalls !== undefined
        ? { parallel_tool_calls: Boolean(genOpts.parallelToolCalls) }
        : {}),
      ...
      try {
        ...
        const dbgMsgs = (body.messages as OpenAIChatMessage[]).map((m) => {
          const toolCalls = Array.isArray(m.tool_calls)
            ? (m.tool_calls as Array<ToolCall>).map((tc) => ({
                id: tc.id,
                function: {
                  name: tc.function?.name,
                  arguments: summarize(tc.function?.arguments),
                },
              }))
            : undefined;
          return {
      ...
      const toolCalls: NormalizedToolCall[] | undefined = (() => {
        ...
        if (msgShape?.function_call) {
          return [
            {
              id: stableToolCallId(
                msgShape.function_call.name,
                msgShape.function_call.arguments,
              ),
              name: msgShape.function_call.name,
              arguments: safeJson(msgShape.function_call.arguments),
            },
          ];
        }
        return undefined;
  ...
  return {
    ...
    ...(opts.responseModel
      ? { meta: { responseModel: opts.responseModel } }
      : {}),
    generate,
  };
}
...
function toJsonSchema(schema: unknown): Record<string, unknown> {
  ...
  if (t === "ZodObject") {
    ...
    for (const [key, val] of Object.entries(shape ?? {})) {
      ...
      const innerTypeName = (val as ZodLike)?._def?.typeName as
        | string
        | undefined;
  ...
  for (const p of parts) {
    ...
    if (t === "image_url") {
      ...
      if (typeof iu === "string") {
        ...
        out.push({
          type: "image_url",
          image_url: { url: (iu as Record<string, unknown>).url as string },
        });
      }
      continue;
    }
...
function mapUsage(u: unknown) {
  ...
  const total =
    obj.total_tokens ?? Number(prompt ?? 0) + Number(completion ?? 0);
  return {
