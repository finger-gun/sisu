 ...
 */
export function toolCallInvariant(
  opts: ToolCallInvariantOptions = {},
): Middleware {
  ...
  return async (ctx: Ctx, next) => {
      ...
      for (const tc of tcs) {
        ...
        const found = after.some(
          (mm) =>
            mm?.role === "tool" &&
            (id
              ? (mm as { tool_call_id?: string }).tool_call_id === id
              : (mm as { name?: string }).name === name),
        );
    ...
    if (missing.length > 0) {
      const payload = missing.map((m) => ({
        assistantIndex: m.assistantIndex,
        tool_call_id: m.toolCallId,
        name: m.name,
      }));
      if (strict) {
        throw new Error(
          `[invariants] Missing tool responses for tool_calls: ${JSON.stringify(payload)}`,
        );
      } else {
        ctx.log.warn?.(
          "[invariants] Missing tool responses for tool_calls",
          payload,
        );
      }
    }
