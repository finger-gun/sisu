import type { Middleware, Message, ModelResponse } from "@sisu-ai/core";
export const reactToolLoop = (): Middleware => {
  return async (ctx, next) => {
    ...
    if (toolMatch && inputMatch) {
      ...
      let args: unknown = inputMatch[1].trim();
      try {
      ...
      ctx.log.debug?.(
        "[react] no tool action parsed; appending assistant message",
      );
