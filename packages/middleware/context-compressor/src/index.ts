...
export const contextCompressor = (
  ...
  return async (ctx, next) => {
    ...
    ctx.model = wrapModelWithCompression(
      original,
      { maxChars, keepRecent, summaryMaxChars, recentClampChars },
      ctx,
    );
    await next();
  };
...
function wrapModelWithCompression(
  model: LLM,
  cfg: Required<ContextCompressorOptions>,
  ctx: Ctx,
): LLM {
  const origGenerate = model.generate.bind(model);

  // Provide a generate function compatible with LLM overloads.
  const wrappedGenerate = ((messages: Message[], genOpts?: GenerateOptions) => {
    ...
    if (genOpts?.stream) {
      try {
        ...
        ctx.log.warn?.(
          "[context-compressor] clampRecent failed (stream path); proceeding",
          e,
        );
      }
    ...
    return (async () => {
      try {
        ...
        if (!ctx.state.__compressing && approxChars(messages) > cfg.maxChars) {
          ctx.log.info?.(
            "[context-compressor] compressing conversation context",
          );
        ...
        ctx.log.warn?.(
          "[context-compressor] failed to compress; proceeding uncompressed",
          e,
        );
      }
...
function approxChars(messages: Message[]): number {
  ...
  for (const m of messages) {
    ...
    else if (Array.isArray(c)) n += JSON.stringify(c).length;
  }
  return n;
}
...
async function compressMessages(
  ...
  const headText = sliceAndFlatten(
    head,
    cfg.summaryMaxChars * HEAD_TO_SUMMARY_RATIO,
  );
  const prompt = [
    {
      role: "system",
      content:
        "You are a compression assistant. Summarize the following conversation and tool outputs into a compact bullet list of established facts and extracted citations (URLs). Keep it under the specified character budget. Do not invent facts.",
    },
    {
      role: "user",
      content: `Character budget: ${cfg.summaryMaxChars}. Include a section "Citations:" listing unique URLs.\n\nConversation to compress:\n${headText}`,
    },
  ] as Message[];

  const res = await gen(prompt, { toolChoice: "none", signal: ctx.signal });
  const summary = String(res?.message?.content ?? "").slice(
    0,
    cfg.summaryMaxChars,
  );
...
function clampRecent(
  ...
  for (
    let i = Math.max(0, out.length - (cfg.keepRecent + 4));
    i < out.length;
    i++
  ) {
    const m = out[i];
    ...
    if (m.role === "tool") {
      ...
      if (clamped !== c) {
        m.content = clamped;
        ctx.log.debug?.("[context-compressor] clamped tool message", {
          idx: i,
          before: c.length,
          after: clamped.length,
        });
      }
    } else if (c.length > limit * 2) {
      m.content = c.slice(0, limit * 2);
      ctx.log.debug?.("[context-compressor] truncated long message", {
        idx: i,
        before: c.length,
        after: m.content.length,
      });
    }
  }
  return out;
...
function findPrevAssistantWithToolCalls(
  messages: Message[],
  start: number,
): number {
