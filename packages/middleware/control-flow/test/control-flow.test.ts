...
test('branch routes to onTrue and onFalse', async () => {
  ...
  expect(ctxT.state.path).toBe('T');
  expect(ctxF.state.path).toBe('F');
});
...
test('switchCase routes by key and uses fallback', async () => {
  ...
  expect(ctxA.state.hit).toBe('A');
  expect(ctxX.state.hit).toBe('fallback');
});
...
test('parallel clones context per branch and merges results', async () => {
  ...
  expect(ctx.state.results).toEqual([1, 2]);
});
...
test('graph throws on excessive steps (cycle)', async () => {
  ...
  await expect(handler(makeCtx())).rejects.toThrow(/step limit exceeded/);
});
