...
export class Server<CtxT extends Ctx & HttpEnvelope<CtxT>> {
  ...
  constructor(
    ...
    this.createCtx =
      opts.createCtx ?? ((req, res) => ({ req, res }) as unknown as CtxT);
  }
      ...
      headers: {
        "user-agent":
          typeof headers["user-agent"] === "string"
            ...
            : undefined,
        accept:
          typeof headers["accept"] === "string" ? headers["accept"] : undefined,
        "content-type":
  ...
  listen(cb?: () => void) {
    ...
    if (printBanner) {
      ...
      this.emitter.emit("listening", {
        url,
        address: addr as string | AddressInfo | null | undefined,
      });
    }
    return this.server;
  }
  ...
  on(
    event: "request",
    handler: (e: { method: string; url: string }) => void,
  ): this;
  on(
    event: "response",
    handler: (e: {
  ...
  on(event: string, handler: (...args: EmitterHandlerArgs) => void): this {
  ...
  once(event: string, handler: (...args: EmitterHandlerArgs) => void): this {
  ...
  off(event: string, handler: (...args: EmitterHandlerArgs) => void): this {
