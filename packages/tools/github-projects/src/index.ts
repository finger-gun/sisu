...
function resolveGraphQLEndpoint(): string {
  ...
  const host =
    firstConfigValue(["GITHUB_ENTERPRISE_HOSTNAME"]) ||
...
async function ghGraphQL(
  ...
  if (!token)
    throw new Error(
      "Missing GitHub token. Set GITHUB_ACCESS_TOKEN or GITHUB_TOKEN.",
    );
  ...
  if (!res.ok)
    throw new Error(
      `GitHub GraphQL HTTP ${res.status}: ${text?.slice(0, 500)}`,
    );
  ...
  if ((json as { errors?: unknown }).errors)
    throw new Error(
      `GitHub GraphQL error: ${JSON.stringify((json as { errors?: unknown }).errors)}`,
    );
  return json;
}
...
async function listProjectIssues(ctx: ToolContext): Promise<IssueSummary[]> {
  ...
  const nodes =
    (
      json as {
        data?: {
          node?: {
            items?: { nodes?: Array<{ id?: string; content?: unknown }> };
          };
        };
      }
  ...
  for (const it of nodes) {
    const node = it as { id?: string; content?: unknown };
    const c = node.content as
      | { id?: string; title?: string; state?: string; url?: string }
      | undefined;
    if (c?.id && c?.title && node.id)
      out.push({
        id: c.id,
        itemId: node.id,
        title: c.title,
...
async function listStatusOptions(ctx: ToolContext): Promise<StatusOption[]> {
  ...
  const json = await ghGraphQL(query, { projectId }, ctx);
  const fields =
    (json as { data?: { node?: { fields?: { nodes?: unknown[] } } } })?.data
      ?.node?.fields?.nodes ?? [];
  const statusField = fields.find(
    (f) =>
      (f as { __typename?: string; name?: string })?.__typename ===
...
async function updateIssue(
  ...
  `;
  const input: { id: string; title?: string; body?: string } = { id: issueId };
...
export const listGitHubIssues: Tool<Record<string, never>> = {
  name: "listGitHubIssues",
  description:
    "List all issues in the configured GitHub Project (Projects v2). Returns id, title, state, url.",
  ...
  handler: async (_args, ctx) => {
    try {
      ...
      return `Failed to list issues: ${e instanceof Error ? e.message : String(e)}`;
    }
  },
};
...
export const getGitHubIssueDetails: Tool<{ issueId: string }> = {
  ...
  handler: async ({ issueId }, ctx) => {
    if (!issueId.startsWith("I_"))
      return "Invalid or missing issue ID. Provide a value like I_123.";
    try {
      const d = await getIssueDetails(issueId, ctx);
      if (!d) return `No details found for issue ${issueId}.`;
      return d;
    } catch (e: unknown) {
      return `Failed to fetch issue details: ${e instanceof Error ? e.message : String(e)}`;
    }
  },
};

export const listGitHubProjectColumns: Tool<Record<string, never>> = {
  name: "listGitHubProjectColumns",
  description:
    "List Status column options for the project with their IDs (usable in moveGitHubIssueToColumn).",
  schema: z.object({}).strict(),
  handler: async (_args, ctx) => {
    try {
      ...
      return `Failed to fetch columns: ${e instanceof Error ? e.message : String(e)}`;
    }
  },
};

export const moveGitHubIssueToColumn: Tool<{
  issueId: string;
  columnId: string;
}> = {
  name: "moveGitHubIssueToColumn",
  description:
    "Move an issue to a specific Status option (column) in the configured project.",
  ...
  handler: async ({ issueId, columnId }, ctx) => {
    ...
    try {
      ...
      return `Failed to move issue: ${e instanceof Error ? e.message : String(e)}`;
    }
  },
};

export const updateGitHubIssue: Tool<{
  issueId: string;
  title?: string;
  body?: string;
}> = {
  ...
  handler: async ({ issueId, title, body }, ctx) => {
    ...
    if (title === undefined && body === undefined)
      return "Specify at least one of title or body to update.";
    try {
      const res = await updateIssue(issueId, title, body, ctx);
      return { ok: true, id: res.id, title: res.title, body: res.body };
    } catch (e: unknown) {
      return `Failed to update issue: ${e instanceof Error ? e.message : String(e)}`;
    }
  },
};
