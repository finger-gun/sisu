...
export const wikipedia: Tool<WikipediaArgs> = {
  ...
  description:
    "Fetch a Wikipedia page summary, HTML, or related pages given an approximate title. Defaults to summary.",
  schema: z.object({
...
function resolveBaseUrl(ctx: ToolContext | undefined, lang?: string): string {
  ...
  const langFromFlags =
    firstConfigValue(["WIKIPEDIA_LANG", "WIKI_LANG"]) || lang;
...
async function fetchHtml(baseUrl: string, title: string): Promise<string> {
  ...
  if (!ct.toLowerCase().includes("text/html"))
    throw new Error(
      `Wikipedia html returned non-HTML content (content-type: ${ct})`,
    );
  return raw;
}
...
async function fetchRelated(
  baseUrl: string,
  title: string,
): Promise<WikipediaRelatedResult> {
