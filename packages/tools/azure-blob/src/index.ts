...
function resolveServiceFromCtx(ctx: ToolContext): BlobServiceClientType {
  ...
  if (!conn)
    throw new Error(
      "AZURE_STORAGE_CONNECTION_STRING is not set (or provide via ctx.deps.azureBlob.connectionString)",
    );
...
const base = {
  containerClient(ctx: ToolContext, container: string): ContainerClient {
...
export const azureListBlobsDetailed: Tool<{
  container: string;
  prefix?: string;
}> = {
  name: "azureListBlobsDetailed",
  description:
    "List blobs with details (e.g., lastModified) in a container, optionally filtered by prefix.",
  schema: z.object({ container: z.string(), prefix: z.string().optional() }),
  handler: async ({ container, prefix }, ctx) => {
    const iter = base.containerClient(ctx, container).listBlobsFlat({ prefix });
...
export const azureUploadBlob: Tool<{
  container: string;
  blobName: string;
  content: string;
}> = {
...
export const azureSetMetadata: Tool<{
  container: string;
  blobName: string;
