...
test("s3GetObject handles Readable streams", async () => {
  const stream = Readable.from([Buffer.from("a"), Buffer.from("b")]);
  const client = { getObject: vi.fn(async () => ({ Body: stream })) };
  const out: any = await s3GetObject.handler(
    { bucket: "b", key: "k" } as any,
    ctxWith(client),
  );
...
test("s3ListObjectsDetailed returns keys with lastModified", async () => {
  const client = {
    listObjectsV2: vi.fn(async () => ({
      Contents: [
        { Key: "a", LastModified: new Date("2020-01-01") },
        { Key: "b", LastModified: new Date("2021-07-01") },
      ],
    })),
  };
...
test("s3ListObjectsDetailed includes size when present", async () => {
  const client = {
    listObjectsV2: vi.fn(async () => ({ Contents: [{ Key: "a", Size: 10 }] })),
  };
  const out: any = await s3ListObjectsDetailed.handler(
    { bucket: "b" } as any,
    ctxWith(client),
  );
  expect(out).toEqual([{ key: "a", size: 10 }]);
});

test("s3PutObject obeys allowWrite flag", async () => {
  const putObject = vi.fn(async () => ({}));
  const client = { putObject };
  const allow = (await s3PutObject.handler(
  ...
  const deny = (await s3PutObject.handler(
    { bucket: "b", key: "k", content: "x" } as any,
    ctxWith(client, false),
  )) as any;
...
test("s3DeleteObject obeys allowWrite flag", async () => {
  ...
  const allow = (await s3DeleteObject.handler(
    { bucket: "b", key: "k" } as any,
    ctxWith(client, true),
  )) as any;
  ...
  const deny = (await s3DeleteObject.handler(
    { bucket: "b", key: "k" } as any,
    ctxWith(client, false),
  )) as any;
  expect(deny.ok).toBe(false);
...
test("s3GetObjectMetadata returns metadata map", async () => {
  const client = {
    headObject: vi.fn(async () => ({ Metadata: { foo: "bar" } })),
  };
  const out: any = await s3GetObjectMetadata.handler(
    { bucket: "b", key: "k" } as any,
    ctxWith(client),
  );
