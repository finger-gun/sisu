...
export const webFetch: Tool<WebFetchArgs> = {
  name: "webFetch",
  description:
    "Fetch a web page by URL and return text, HTML, or JSON. Defaults to text extraction for HTML.",
  schema: z.object({
    url: z.string().url(),
  ...
  handler: async (
    { url, format = "text", maxBytes, respectRobots },
    ctx,
  ): Promise<WebFetchResult> => {
    const ua =
      firstConfigValue(["WEB_FETCH_USER_AGENT", "HTTP_USER_AGENT"]) ||
      "SisuWebFetch/0.1 (+https://github.com/finger-gun/sisu)";
    const capEnv = firstConfigValue(["WEB_FETCH_MAX_BYTES"]);
    const cap = Number(
      maxBytes ?? (capEnv !== undefined ? Number(capEnv) : 500_000),
    );
    ...
    if (respect) {
      const decision = await robotsDecision(url, ua).catch(
        () => ({ allowed: true }) as RobotsDecision,
      );
      if (!decision.allowed) {
    ...
    const finalUrl = (res as { url?: string }).url || undefined;
    if (!res.ok) {
      return {
        url,
        finalUrl,
...
const robotsCache = new Map<
  string,
  { ts: number; rules: RobotsRules | null }
>();

async function robotsDecision(
  targetUrl: string,
  userAgent: string,
...
function evaluateRobotsDetailed(
  rules: RobotsRules,
  userAgent: string,
  ...
  const matching = rules.groups
    ...
    .filter((x) => !!x.matchedAgent) as Array<{
    g: RobotsGroup;
    matchedAgent: string;
  }>;
...
function htmlToText(html: string): string {
  ...
  s = s.replace(/<(br|BR)\s*\/?>(\n)?/g, "\n");
  s = s.replace(
    /<\/(p|div|section|article|h[1-6]|li|ul|ol|header|footer|main)>/gi,
    "\n",
  );
