...
interface DDGIcon { Height?: string | number; Width?: string | number; URL?: string; }
interface DDGTopic { FirstURL: string; Text: string; Icon?: DDGIcon; Result?: string; }
interface DDGTopicGroup { Name?: string; Topics: DDGTopic[]; }
...
export const duckDuckGoWebSearch: Tool<DuckDuckGoSearchArgs> = {
  ...
  handler: async ({ query }) => {
    ...
    const results: DuckDuckGoResultItem[] = flat
      ...
      .map((t) => ({ title: String(t.Text), url: String(t.FirstURL), iconUrl: toAbsoluteIcon(t.Icon) }))
      // Deduplicate by URL (DDG sometimes repeats items across groups)
      .reduce<DuckDuckGoResultItem[]>((acc, cur) => acc.find((x) => x.url === cur.url) ? acc : acc.concat(cur), [])
      .slice(0, 8);
